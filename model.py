...
class Model(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        # Add more reasoning layers
        self.self_att2 = BiAttention(config.hidden*2, 1-config.keep_prob)
        self.linear_att2 = nn.Sequential(nn.Linear(config.hidden*8, config.hidden),nn.ReLU())
        self.self_att3 = BiAttention(config.hidden*2, 1-config.keep_prob)
        self.linear_att3 = nn.Sequential(nn.Linear(config.hidden*8, config.hidden),nn.ReLU())
        #	
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        # Add more reasoning layers
        output_t = self.linear_att2(output_t)
        output_t = self.self_att2(output_t, output_t, context_mask)
        output_t = self.linear_att3(output_t)
        output_t = self.self_att3(output_t, output_t, context_mask)
        #
